    ...
    var body: some View {
        VStack {
            ...
            Text("moon travel thingys = \(MoonPoint)")
                ...
                if counter >= 50 {Button("upgrade (50)", action: {counter -= 50; cpc += 1})
                        .disabled(false)
                } else if counter <= 49 {
                    Button("upgrade (50)", action: {counter -= 0; cpc += 0})
                        .disabled(true)
            ...
            if counter >= 1000 {
                Button("click me to go to moon! (1k points", action: {counter -= 1000; MoonPoint += 1})
                    .disabled(false)
